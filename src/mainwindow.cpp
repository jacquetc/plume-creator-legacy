...

{
    ...
    hub = new Hub(this);
    connect(hub, SIGNAL(openProjectSignal()), this,
            SLOT(openProjectSlot()));
    connect(hub, SIGNAL(closeProjectSignal()), this,
            SLOT(closeProjectSlot()));
    connect(this, SIGNAL(closeAllChildrenWindowsSignal()), hub,
            SIGNAL(closeAllChildrenWindowsSignal()));
    connect(hub, SIGNAL(textAlreadyChangedSignal(bool)), this,
            SLOT(textAlreadyChangedSlot(bool)));
    connect(hub, SIGNAL(showStatusBarMessageSignal(QString,int)), ui->bar,
    ...
    if (m_firstStart_checkDisplay &&
        (QApplication::desktop()->availableGeometry().height() < 650)) {
        QMessageBox firstStart_checkDisplay;
        firstStart_checkDisplay.setWindowTitle(tr("Small screen size detected"));
        firstStart_checkDisplay.setText(tr(
                                            "<center><b>A small screen has been detected</b></center>"
...
void MainWindow::createMenuBar()
{
    ...
    connect(menu, SIGNAL(createNewStructureSignal(QHash<QString,int>)), mainTree,
            SLOT(createNewStructure(QHash<QString,int>)));

    connect(menu, SIGNAL(exitSignal()), this,
            SLOT(close()));
    connect(menu, SIGNAL(setDisplayModeSignal(QString,bool)), this,
            SLOT(setDisplayMode(QString,bool)));

    connect(menu, SIGNAL(launchCheckUpdateSignal(QString)), this,
            SLOT(launchSlimUpdater(QString)));
    connect(menu, SIGNAL(applyStyleSheetSignal()), this,
            SLOT(giveStyle()));
    connect(menu, SIGNAL(applyConfigSignal()), hub,
            SLOT(applyConfig()));
...
void MainWindow::createMenuDock()
{
    ...
    //    menuDock->setAllowedAreas(Qt::RightDockWidgetArea |
    // Qt::LeftDockWidgetArea);
    //    menuDock->setFeatures(QDockWidget::DockWidgetFloatable |
    ...
    //    connect(menu, SIGNAL(openProjectSignal(QFile*)), this,
    // SLOT(openProjectSlot(QFile*)));

    //    connect(menu,SIGNAL(exitSignal()), this, SLOT(close()));
    //    connect(menu, SIGNAL(closeProjectSignal()), this,
    // SLOT(closeProjectSlot()));
...
void MainWindow::createTreeDock()
{
    ...
    mainTree->setHub(hub);
    connect(mainTree->mainTreeAbstractModel(),
            SIGNAL(textAndNoteSignal(int,QString)),
            this,
            SLOT(textSlot(int,QString)));
    ...
    connect(dockedTree, SIGNAL(textAndNoteSignal(int,QString)), this,
            SLOT(textSlot(int,QString)));
    ...
    connect(dockedTrashTree, SIGNAL(textAndNoteSignal(int,QString)), this,
            SLOT(textSlot(int,QString)));
    ...
    //    connect(treeDock, SIGNAL(visibilityChanged(bool)), this,
    // SLOT(checkHiddenDocks()));
    treeDock->show();
}
...
void MainWindow::createNoteDock()
{
    ...
    connect(noteDock, SIGNAL(visibilityChanged(bool)), this,
            SLOT(checkHiddenDocks()));
    connect(noteDock, SIGNAL(dockLocationChanged(Qt::DockWidgetArea)), this,
            SLOT(changeOrientationOfNoteDock(Qt::DockWidgetArea)));
...
{
    if ((noteDockArea == Qt::LeftDockWidgetArea) ||
        (noteDockArea == Qt::RightDockWidgetArea)) {
        //        noteDockLayout->setDirection(QBoxLayout::TopToBottom);
        noteSplitter->setOrientation(Qt::Vertical);
    }
    else if ((noteDockArea == Qt::TopDockWidgetArea) ||
             (noteDockArea == Qt::BottomDockWidgetArea)) {
...
void MainWindow::createStatusBar()
{
    ...
    connect(hub, SIGNAL(projectWordCount(int)), this,
            SLOT(updateProjectWCLabel(int)));
    ...
    connect(hub, SIGNAL(chapterWordCount(int)), this,
            SLOT(updateChapterWCLabel(int)));
    connect(hub, SIGNAL(currentSheetWordCount(int)), this,
            SLOT(updateCurrentSheetWCLabel(int)));
...
void MainWindow::createDocksToolBar()
{
    ...
    connect(menu, SIGNAL(launchOutlinerSignal()),     this,
            SLOT(launchWorkbench()));
    ...
    treeDockButton =
        new OrientationButton(QIcon(":/pics/view-list-tree-90.png"), tr("Project"));
    ...
    noteDockButton = new OrientationButton(QIcon(":/pics/im-status-message-edit-90.png"),
                                           tr("Notes"));
    ...
    noteDockAct = new QAction(QIcon(":/pics/im-status-message-edit.png"),
                              tr("Notes"),
                              this);
    ...
    toolDockButton =
        new OrientationButton(QIcon(":/pics/preferences-system-90.png"), tr("Tools"));
    ...
    attendDockButton =
        new OrientationButton(QIcon(":/pics/meeting-organizer-90.png"),
                              tr("Mise en scène"));
    ...
    attendDockAct = new QAction(QIcon(":/pics/meeting-organizer.png"),
                                tr("Mise en scène"),
                                this);
    ...
    OrientationButton *workbenchButton =
        new OrientationButton(QIcon(":/pics/view-time-schedule-90.png"), tr("Outliner"));
    ...
    workbenchAct = new QAction(QIcon(":/pics/view-time-schedule.png"),
                               tr("Workbench"),
                               this);
    ...
    OrientationButton *fullscreenButton =
        new OrientationButton(QIcon(":/pics/view-fullscreen-90.png"), tr("Fullscreen"));
    ...
    fullscreenAct = new QAction(QIcon(":/pics/view-fullscreen.png"),
                                tr("Fullscreen"),
                                this);
...
void MainWindow::setDisplayMode(QString mode, bool isToolBarInStatusBar)
{
    ...
    if (mode == "desktop") {
        if (displayMode == "netbook") settings.setValue("netbook_wmState", saveState(1));

        disconnect(attendDock, SIGNAL(visibilityChanged(bool)), this,
                   SLOT(attendDockHidesOthers(bool)));
        disconnect(noteDock, SIGNAL(visibilityChanged(bool)), this,
                   SLOT(noteDockHidesOthers(bool)));
    ...
    if (mode == "netbook") {
        if (displayMode == "desktop") settings.setValue("desktop_wmState", saveState(1));
        ...
        connect(attendDock, SIGNAL(visibilityChanged(bool)), this,
                SLOT(attendDockHidesOthers(bool)));
        connect(noteDock, SIGNAL(visibilityChanged(bool)), this,
                SLOT(noteDockHidesOthers(bool)));
...
void MainWindow::openExternalProject(QString externalFile)
{
    ...
    //                                          "The file path may contain
    // special characters.\n"
    //                                          "Please rename the folders and
    // file without these characters :\n"
    //                                          "Ã©,Ã¨,Ã ,Ã¯,Ã§ et cetera... \n"
    ...
    isExternalProjectOpeningBool = true; // needed to avoid the opening of the
                                         // project manager at startup
...
void MainWindow::openProjectSlot()
{
    ...
    connect(mainTree->mainTreeAbstractModel(),
            SIGNAL(applySettingsFromDataSignal()),
            dockedTree,
            SLOT(applySettingsFromData()),
            Qt::UniqueConnection);
    connect(dockedTree,
            SIGNAL(modifyFlagsForDropsSignal(QString)),
            proxy,
            SLOT(modifyFlagsForDrops(QString)),
            Qt::UniqueConnection);
    connect(proxy,
            SIGNAL(resetAbsModelSignal()),
            mainTree->mainTreeAbstractModel(),
            SLOT(resetAbsModel()),
            Qt::UniqueConnection);
    connect(proxy,
            ...
            dockedTree,
            SLOT(adaptColumn()),
            Qt::UniqueConnection); // resize column to content
    connect(dockedTree,
            SIGNAL(contentSizeChanged(int)),
            this,
            SLOT(resizeMinimumTreeDockWidth(int)),
            Qt::UniqueConnection);
    ...
    connect(mainTree->mainTreeAbstractModel(),
            SIGNAL(displayBadgeSignal(bool)),
            proxy,
            SLOT(displayBadgeSlot(bool)),
            Qt::UniqueConnection);
    connect(mainTree->mainTreeAbstractModel(),
            SIGNAL(displayBadgeSignal(bool)),
            trashProxy,
    ...
    connect(dockedTree,
            SIGNAL(currentOpenedSheetSignal(int)),
            mainTree->mainTreeAbstractModel(),
            SLOT(modifyDataForOpenedSheetMarker(int)),
            Qt::UniqueConnection);
    connect(dockedTrashTree,
            SIGNAL(currentOpenedSheetSignal(int)),
            mainTree->mainTreeAbstractModel(),
...
void MainWindow::textSlot(int number, QString action)
{
    if (action == "open") {
        ...
        MainTextDocument *textDoc = hub->project()->findChild<MainTextDocument *>("textDoc_" + QString::number(
                                                                                      number));
        MainTextDocument *synDoc =  hub->project()->findChild<MainTextDocument *>("synDoc_" + QString::number(
                                                                                      number));
        MainTextDocument *noteDoc = hub->project()->findChild<MainTextDocument *>("noteDoc_" + QString::number(
                                                                                      number));


        QString name =
            hub->project()->mainTree_domElementForNumberHash().value(number).attribute(
                "name");
        ...
        //        qDebug() << "added objectname value : " <<
        // string.setNum(number,10);
        ...
        connect(tab, SIGNAL(charFormatChangedSignal(QTextCharFormat)), menu,
                SIGNAL(charFormatChangedSlotSignal(QTextCharFormat)));
        connect(menu, SIGNAL(styleSelectedSignal(int)), tab,
                SIGNAL(changeTextStyleSignal(int)));
        connect(tab, SIGNAL(setStyleSelectionSignal(int)), menu,
                SIGNAL(setStyleSelectionSignal(int)));
        connect(tab, SIGNAL(manageStylesSignal()), menu,
                SLOT(manageStyles()));


        // connect note & syn to maintree (for Outliner) :

        connect(noteStack, SIGNAL(noteFocusOutSignal()), this,
                SIGNAL(updateOutlinerSignal()));
        connect(synStack, SIGNAL(noteFocusOutSignal()), this,
                SIGNAL(updateOutlinerSignal()));

        // connect maintree to note & syn (for Outliner) :
        connect(this, SIGNAL(applySynNoteFontConfigSignal()), noteStack,
                SLOT(applyNoteFontConfig()));
        connect(this, SIGNAL(applySynNoteFontConfigSignal()), synStack,
                SLOT(applySynFontConfig()));

        // launch autosaving :
        if (!hub->project()->isProjectOpened()) autosaveTimer();


        //        QString debug;
        //        qDebug() << "cursorPosition tab : " <<
        // debug.setNum(textCursorPosition);
        //        qDebug() << "cursorPosition syn : " <<
        // debug.setNum(synCursorPosition);
        //        qDebug() << "cursorPosition note : " <<
        // debug.setNum(noteCursorPosition);
        ...
        connect(tab, SIGNAL(activateSpellcheckSignal(bool)), this,
                SLOT(activateSpellCheck(bool)));
        ...
        if (!numList->contains(number)) return;


        QString  string;
        TextTab *tab =
            ui->mainTabWidget->findChild<TextTab *>("tab_" + string.setNum(number, 10));

        // disconnect edit menu to tab
        disconnect(menu, SIGNAL(widthChangedSignal(int)), tab,
                   SLOT(changeWidthSlot(int)));


        disconnect(tab, SIGNAL(charFormatChangedSignal(QTextCharFormat)), menu,
                   SIGNAL(charFormatChangedSlotSignal(QTextCharFormat)));
...
void MainWindow::setConnections()
{
    ...
    connect(ui->mainTabWidget, SIGNAL(currentChanged(int)), noteLayout,
            SLOT(setCurrentIndex(int)));
    connect(ui->mainTabWidget, SIGNAL(currentChanged(int)), synLayout,
            SLOT(setCurrentIndex(int)));
    connect(ui->mainTabWidget, SIGNAL(currentChanged(int)), this,
            SLOT(tabChangeSlot(int)));
    connect(ui->mainTabWidget, SIGNAL(tabCloseRequested(int)), this,
            SLOT(tabCloseRequest(int)));
    connect(mainTree, SIGNAL(nameChangedSignal(QString,int)), this,
            SLOT(tabRenamingSlot(QString,int)));
    connect(menu, SIGNAL(openProjectNumberSignal(int)), this,
            SLOT(setProjectNumberSlot(int)));
    connect(menu, SIGNAL(saveProjectSignal()), this,
            SLOT(saveAllDocsSlot()));
    ...
    connect(ui->mainTabWidget, SIGNAL(currentChanged(int)), this,
            SLOT(setCurrentAttendList(int)));


    // for previous and next texts :
    connect(ui->mainTabWidget, SIGNAL(currentChanged(int)), this,
            SLOT(showPrevAndNextTexts()));
    connect(mainTree->mainTreeAbstractModel(), SIGNAL(treeStructureChanged()), this,
            SLOT(showPrevAndNextTexts()));
}
...
{
    disconnect(ui->mainTabWidget, SIGNAL(currentChanged(int)), this,
               SLOT(tabChangeSlot(int)));
    ...
    //    qDebug() << "tabCloseRequest textName :" <<
    // textWidgetList->at(tabNum)->objectName() << "----------- saved :" <<
    // textBool;
    //    qDebug() << "tabCloseRequest noteName :" <<
    // noteWidgetList->at(tabNum)->objectName() << "----------- saved :" <<
    // noteBool;
    //    qDebug() << "tabCloseRequest synName :" <<
    // synWidgetList->at(tabNum)->objectName() << "----------- saved :" <<
    // synBool;
    ...
    //    connect(mainTabWidget, SIGNAL(currentChanged(int)),
    // this,SLOT(tabChangeSlot(int)),Qt::UniqueConnection);
}
...
void MainWindow::closeAllDocsSlot()
{
    ...
    for (int i = nameList->size() - 1; i >= 0; --i) {
        ...
        //        qDebug() << "closeAllRequest textName :" <<
        // textWidgetList->at(i)->objectName() << "----------- saved :" <<
        // textBool;
        //        qDebug() << "closeAllRequest noteName :" <<
        // noteWidgetList->at(i)->objectName() << "----------- saved :" <<
        // noteBool;
        //        qDebug() << "closeAllRequest synName :" <<
        // synWidgetList->at(i)->objectName() << "----------- saved :" <<
        // synBool;
...
void MainWindow::saveAllDocsSlot()
{
    for (int i = nameList->size() - 1; i >= 0; --i) {
        ...
        //        qDebug() << "tabSaveRequest textName nÂ° " << i << " ---> " <<
        // textWidgetList->at(i)->objectName() << "----- saved :" << textBool;
        //        qDebug() << "tabSaveRequest noteName nÂ° " << i << " ---> " <<
        // noteWidgetList->at(i)->objectName() << "----- saved :" << noteBool;
        //        qDebug() << "tabSaveRequest synName  nÂ° " << i << " ---> " <<
        // synWidgetList->at(i)->objectName() << "----- saved :" << synBool;
    }
}
...
void MainWindow::tabRenamingSlot(QString newName, int number)
{
    ...
    //        disconnect(mainTabWidget, SIGNAL(currentChanged(int)),
...
void MainWindow::readSettings()
{
    ...
    m_firstStart_checkDisplay =
        settings.value("firstStart_checkDisplay", true).toBool();
    checkScreenResAtStartupBool =
        settings.value("checkScreenResAtStartup", true).toBool();
    settings.endGroup();
...
void MainWindow::writeSettings()
{
    ...
    if (settings.value("displayMode",
                       "desktop").toString() ==
        "netbook") settings.setValue("netbook_wmState", saveState(1));
    else if (settings.value("displayMode",
                            "desktop").toString() ==
             "desktop") settings.setValue("desktop_wmState", saveState(1));
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    //        //        systemTray->show();
    //        //        systemTray->showMessage("Plume Creator", tr("Your
    // project was successfully saved."), QSystemTrayIcon::Information, 3000);
...
void MainWindow::applyConfig()
{
    ...
    if (hub->project()->isProjectOpened()) configTimer();
    ...
    if (settings.value("Settings/numberSymbolIsComma",
                       false).toBool()) numberSymbol =  ",";
    else numberSymbol =  " ";
...
{
    if (ui->mainTabWidget->count() == 0) return;
    ...
    connect(stats,
            SIGNAL(timerSignal(QString)),
            fullEditor,
            SLOT(setTimer(QString)));
    connect(fullEditor,
            SIGNAL(manageStylesSignal()),
            menu,
            SLOT(manageStyles()));
    connect(fullEditor,
            SIGNAL(newSheetSignal(int)),
            mainTree->mainTreeAbstractModel(),
            SLOT(addItemNext(int)));
    connect(fullEditor,
            SIGNAL(openSheetOnMainWindow(int,int)),
            this,
            SLOT(openSheet(int,int)));
}
...
{
    if (isProjectOpened == false) return;
    ...
    connect(workbench, SIGNAL(applySynNoteFontConfigSignal()), this,
            SIGNAL(applySynNoteFontConfigSignal()), Qt::UniqueConnection);
    connect(this, SIGNAL(updateOutlinerSignal()), workbench,
            SIGNAL(updateOutlinerSignal()),         Qt::UniqueConnection);
    connect(workbench, SIGNAL(destroyed()), this,
            SLOT(killWorkbench()),                  Qt::UniqueConnection);
    connect(this,
            SIGNAL(closeAllChildrenWindowsSignal()),
            workbench,
            SLOT(close()),
...
void MainWindow::showPrevText(bool showPrevTextBool)
{
    ...
    int number = ui->mainTabWidget->currentWidget()->objectName().mid(
        ui->mainTabWidget->currentWidget()->objectName().indexOf("_") + 1).toInt();
...
// ----------------------------------------------------------------------------
void MainWindow::openSheet(int sheetNumber,
                           int textCursorPos,
                           int noteCursorPos,
                           int synCursorPos)
{
    ...
    TextTab *tab =
        ui->mainTabWidget->findChild<TextTab *>("tab_" +
                                                QString::number(hub->project()->
                                                                currentSheetNumber()));

    NoteZone *noteStack =
        this->findChild<NoteZone *>("note_" +
                                    QString::number(
                                        hub->project()->currentSheetNumber()) +
                                    "-NoteZone");
    NoteZone *synStack =
        this->findChild<NoteZone *>("syn_" +
                                    QString::number(
                                        hub->project()->currentSheetNumber()) +
                                    "-NoteZone");
...
void MainWindow::setCurrentAttendList(int tabNum)
{
    ...
    int number = ui->mainTabWidget->widget(tabNum)->objectName().mid(ui->mainTabWidget->widget(
                                                                         tabNum)->objectName().indexOf(
                                                                         "_") +
                                                                     1).toInt();
    hub->project()->setCurrentSheetNumber(number); // updates also the
                                                   // attendance list
...
{
    if (ui->mainTabWidget->count() == 0) return 0;
...
void MainWindow::giveStyle()
{
    ...
    QString customCss =
        "QTreeView {"
        "background-color: " +
        settings.value("MainWindow/projectTreeBackColor", "#dee4ea").toString() + " ;"
                                                                                  "}"
                                                                                  "QTreeView::item {"
                                                                                  "color: "
        + settings.value("MainWindow/projectTreeTextColor", "#000000").toString() + " ;"
                                                                                    "}"

                                                                                    "NoteZone {"
                                                                                    "color: "
        + settings.value("MainWindow/noteZoneTextColor", "#000000").toString() + ";"
                                                                                 "background-color: "
        + settings.value("MainWindow/noteZoneBackColor", "#fff7d7").toString() + ";"
                                                                                 "}"


    ;

    if (!settings.value("Settings/applyCustomColors",
                        true).toBool()) customCss =       "";
...
{
    projectWCLabel->setText(tr("Project: ") +
                            Utils::spaceInNumber(QString::number(count), numberSymbol));
}

void MainWindow::updateBookWCLabel(int count)
{
    bookWCLabel->setText(tr("Book: ") +
                         Utils::spaceInNumber(QString::number(count), numberSymbol));
}

void MainWindow::updateActWCLabel(int count)
{
    if (count == -1) actWCLabel->hide();
    else {
        actWCLabel->show();
        actWCLabel->setText(tr("Act: ") +
                            Utils::spaceInNumber(QString::number(count), numberSymbol));
    }
}
...
{
    if (count == -1) chapterWCLabel->hide();
    else {
        chapterWCLabel->show();
        chapterWCLabel->setText(tr("Chapter: ") +
                                Utils::spaceInNumber(QString::number(count),
                                                     numberSymbol));
    }
}
...
{
    currentWCLabel->setText(tr("Words: ") +
                            Utils::spaceInNumber(QString::number(count), numberSymbol));
}
